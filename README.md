...
`shots/list` errors with `invalid_format` for unparseable `after`/`before`.
...
rows of the event entity no longer contain the curve arrays, so switch to
the commands above for history.
